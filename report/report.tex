...
Our best model achieves a test ROC-AUC of \textbf{0.922} and PR-AUC of \textbf{0.770},
outperforming a TF-IDF + Logistic Regression text baseline (ROC-AUC 0.877, PR-AUC 0.652).
A layer sweep reveals that safety signal peaks at mid-network depth (layer 14)
and slightly degrades in later layers, a finding consistent with the hypothesis
that mid-layers encode rich semantic content while late layers shift toward
...
\begin{enumerate}[noitemsep]
  ...
  \item Extract the hidden state at a target layer $\ell \in \{4, 14, 26\}$.
...
We select three representative layers:
\begin{itemize}[noitemsep]
  \item \textbf{Layer 4} (early): low-level syntactic/lexical features.
  \item \textbf{Layer 14} (mid): hypothesised to encode rich semantic content.
...
Activation probe, Layer 14 (mid),   Linear  & \textbf{0.922} & \textbf{0.770} & 0.660 \\
Activation probe, Layer 14 (mid),   MLP     & 0.912 & 0.728 & 0.636 \\
Activation probe, Layer 26 (late),  Linear  & 0.892 & 0.740 & \textbf{0.672} \\
\midrule
...
The activation probe at layer 14 outperforms the TF-IDF baseline on both
ROC-AUC ($+0.045$) and PR-AUC ($+0.118$), confirming that internal
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\linewidth]{../outputs/figures/layer_sweep_linear.png}
  \caption{ROC-AUC and PR-AUC of the linear probe across three transformer layers.
           Safety signal peaks at layer 14 (mid-network) and does not increase
           monotonically toward the output.}
  \label{fig:layer_sweep}
\end{figure}

Figure~\ref{fig:layer_sweep} reveals a non-monotonic relationship between layer
depth and probe performance. Safety signal is weak in early layers (layer 4),
peaks at the mid-network (layer 14), and slightly degrades in late layers
(layer 26). We hypothesise that:

\begin{itemize}[noitemsep]
  \item \textbf{Early layers} encode predominantly syntactic/lexical features
        with limited semantic discrimination.
  \item \textbf{Mid layers} build rich contextual semantic representations,
        including intent-related features.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.90\linewidth]{../outputs/figures/roc_pr_layer14_linear.png}
  \caption{ROC (left) and Precision-Recall (right) curves for the best activation
           probe (layer 14, linear) versus the TF-IDF text baseline. The activation
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.40\linewidth]{../outputs/figures/confusion_layer14_linear.png}
  \caption{Confusion matrix for the layer 14 linear probe at the optimal threshold
           (0.82), selected on the validation set to maximise F1(harmful).}
  \label{fig:confusion}
\end{figure}

At threshold 0.82, the probe achieves strong recall on the harmful class
while maintaining manageable false alarm rates. The high threshold (0.82 vs.\ the
...
The linear probe (ROC-AUC 0.922) outperforms the MLP (0.912) at layer 14.
This suggests that the harmful vs.\ benign separation in mid-layer activation
space is approximately \emph{linear}, a notable finding given the complexity
of jailbreak attacks. The MLP likely overfits slightly with only 300 harmful
training examples.
...
\begin{table}[H]
\centering
\caption{Effect of training set balance on layer 14 linear probe performance.
...
\begin{table}[H]
\centering
\caption{Probe predictions on selected prompts (layer 14, linear, threshold=0.82).
...
The optimal classification threshold is 0.82 rather than the standard 0.50.
...
\begin{enumerate}
  ...
  \item \textbf{Single architecture and tokenizer.}
        Results are specific to Qwen2.5-1.5B. The safety signal location (layer 14)
  ...
  \item \textbf{Last-token pooling.}
        Mean pooling across all tokens may capture more complete semantic signal.
        This ablation was identified but not completed within the project scope.
...
\begin{lstlisting}
cd src
python extract_features.py --layers 4 14 26 --pool last --batch_size 16
\end{lstlisting}
Saves feature tensors to \texttt{features/qwen2.5-1.5b/layer\_\{4,14,26\}/X\_\{split\}.pt}.
...
python eval_probe.py --layer 14 --probe_type linear --layer_sweep
\end{lstlisting}
...
File & Contents \\
\midrule
\texttt{outputs/checkpoints/qwen2.5-1.5b\_layer14\_linear.pt} & Best probe weights + normalisation stats + threshold \\
...
We demonstrated that a lightweight linear probe trained on frozen mid-layer
activations of Qwen2.5-1.5B can detect harmful and jailbreak prompts with
ROC-AUC 0.922 and PR-AUC 0.770, outperforming a strong TF-IDF text baseline
on both metrics. The layer sweep revealed that safety signal peaks at layer 14
(mid-depth) and does not increase monotonically toward the output, suggesting
that intermediate transformer representations encode the richest semantic
...
The primary limitations are the small minority training set (300 examples),
threshold calibration issues from weighted BCE, and vulnerability to
non-adversarial direct harm prompts. Future work could investigate mean pooling,
multi-layer feature concatenation, or calibration via temperature scaling, as
well as testing robustness against paraphrase-based evasion attacks.
