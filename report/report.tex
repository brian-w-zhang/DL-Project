...
\midrule
Class distribution (EDA) & \texttt{eda\_class\_distribution.png} & \texttt{notebooks/01\_eda.ipynb} \\
Prompt length by class    & \texttt{eda\_prompt\_length.png}      & \texttt{notebooks/01\_eda.ipynb} \\
Adversarial breakdown     & \texttt{eda\_adversarial\_breakdown.png} & \texttt{notebooks/01\_eda.ipynb} \\
\midrule
...
Confusion matrices (side-by-side) & \texttt{confusion\_pooling\_comparison.png} & NB \S\,4 \\
Training curves (L19 linear)  & \texttt{training\_curves\_qwen2.5-1.5b\_layer19\_linear.png} & \texttt{train\_probe.py -{}-layer 19} \\
Hyperparameter sweep          & \texttt{hparam\_sweep\_layer19\_linear.png} & \texttt{hparam\_sweep.py -{}-layer 19} \\
...
\begin{tabular}{lll}
\toprule
 & v1 (original) & v2 (extended) \\
\midrule
Harmful train examples  & 300   & 700 \\
Total train             & 3,000 & 7,000 \\
Internal test (harmful) & 600 (60)   & 2,000 (200) \\
External test           & ---   & WildGuardTest: 1,699 rows (754 harmful) \\
Split design            & 90:10 imbalance & 70/10/20 train/val/test, 90:10 imbalance \\
Extraction hardware     & Apple MPS ($\sim$40 min/method, $\sim$80 min total) & Colab T4 GPU ($\sim$60 min/method, $\sim$120 min total) \\
\bottomrule
\end{tabular}
\end{table}
