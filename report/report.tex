...
To empirically formalize this vulnerability, we binned the entirety of the 600-prompt test set into targeted character-length increments and charted the conditional recall rate of both detection paradigms. Figure~\ref{fig:recall_length} illustrates a stark, undeniable divergence in architectural tracking.

For short, concise statements (under 100 characters), both the TF-IDF baseline and the Layer 19 probe perform with highly comparative, robust accuracy. However, as the character length of the prompt inflates, the efficacy of the surface-level TF-IDF baseline suffers a catastrophic, near-linear degradation. For extremely verbose, heavily obfuscated prompts spanning over 300 characters, the baseline's recall plummets as the density of harmful keywords is mathematically overwhelmed by innocent contextual padding. 

In sharp contrast, the Layer 19 Activation Probe maintains an uncompromising, geometrically flat detection rate uniformly across all mapped prompt length distributions. Because the LLM backbone intrinsically routes hidden state information utilizing densely contextualized self-attention blocks, the mathematical representation of the explicit harm is dynamically reinforced and tracked regardless of how deeply it is buried within the text. This explicitly serves as quantitative validation that intermediate transformer representations natively defend against verbosity-based evasion tactics.
